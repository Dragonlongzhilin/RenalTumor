...
sub.scATAC.harmony <- readRDS("8.Immune/Macrophage/sub.scATAC.harmony.pro.rds")
...
sub.scRNA.harmony <- readRDS("/data/active_data/lzl/RenalTumor-20200713/DataAnalysis-20210803/scRNA/5.Immune/Macrophage/sub.scRNA.harmony.pro.rds")
...
res <- sapply(interest.genes, function(x){
    ...
    write.xlsx(target.genelists, file = paste0("8.Immune/Macrophage/TF.analysis/", x, "_targetGenes.xlsx"), sheetName = c("overlap with hvgs in scRNA-seq", "all target genes"))
    ...
    pdf(paste0('8.Immune/Macrophage/TF.analysis/', celltype, '_', motif_name, '_targets.pdf'), width=5, height=4, useDingbats=FALSE)
    print(p1)
